...
NP Companion is a locally-hosted web application that runs entirely within
...
is enforced by the application's background job scheduler and logged to
the audit trail.
...
The Tailscale network for NP Companion contains exactly two devices: the
...
**No independent ONC certification**: NP Companion is a workflow support
...
excluded from version control via `.gitignore`. The `.env.example` file
documents the required configuration structure without real values.
