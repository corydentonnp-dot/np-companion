...
The main programming language for the entire backend of NP Companion. Choose version 3.11 specifically for maximum library compatibility.
...
1. Go to python.org/downloads and download Python 3.11.x (not 3.12 or higher — some automation libraries lag behind).
...
4. You should see: Python 3.11.x — if you see an error, the PATH was not set correctly. Uninstall and reinstall with the checkbox checked.
...
7. Open Command Prompt and type: node \--version — you should see a version number.

### **1.3  Git for Windows**

Version control — tracks every change you make to your code so you can undo mistakes. Think of it as unlimited Ctrl+Z for your entire project.
...
8. Download and run the installer. Accept all defaults — the default settings are correct for beginners.
...
18. You will now see a Claude icon in your sidebar — clicking it opens a chat panel inside VS Code.
...
20. Run the installer. When asked about installation path, note what folder it installs to — you will need this path later. Default is C:\\Program Files\\Tesseract-OCR\\
...
Creates a secure private network between your phone and your work PC so you can access NP Companion from anywhere. Free for personal use.
...
26. You will also install Tailscale on your work PC and your phone later — all three devices need to be signed into the same Tailscale account for the tunnel to work.

### **1.8  AutoHotkey v2**

The tool that lets scripts control the mouse and keyboard — required for the Order Set Automator and text expansion macros.
...
27. Download AutoHotkey v2 from autohotkey.com (make sure to get v2, not v1 — the syntax is different).
...
31. Note your User Key from the dashboard — you will need this later.

32. Create a new Application in Pushover and note the API Token — this is what identifies NP Companion as the sender.

33. Install the Pushover app on your phone ($5 one-time purchase) and sign in.

### **1.10  DB Browser for SQLite**

A visual tool to look inside your local database — useful for verifying data is being saved correctly and for troubleshooting.
...
| ✅  CHECKPOINT — Test Before Moving On: python \--version shows Python 3.11.x in Command Prompt node \--version shows a version number in Command Prompt git \--version shows a version number in Command Prompt VS Code opens and the Claude extension icon is visible in the sidebar tesseract \--version shows a version number in Command Prompt Tailscale is installed and you are signed into an account AutoHotkey v2 is installed You have a Pushover User Key and API Token saved somewhere safe DB Browser for SQLite opens without errors |
| :---- |
...
| ⚠️  Security Note: Never share any of the information collected in this section publicly or paste it into an AI chat. Store it in a password-protected notes app or a physical notebook kept securely. |
| :---- |

### **2.1  Work PC Local IP Address**

Why you need it: Your work PC's address on the clinic's local network — needed so room computers can reach NP Companion.
...
Why you need it: Critical for the MRN screen reader — the capture region coordinates depend on your resolution.
...
| Open Amazing Charts on your work PC. Open any patient chart. The MRN will appear in the upper corner of the window. You will use the Calibration Tool (Feature 28a) to capture this precisely — for now, just note which corner of the screen it appears in and approximately how large the text is. |
| :---- |
...
Why you need it: Your unique identifier in Pushover — goes in the NP Companion config file.
...
Why you need it: The token specific to the NP Companion app you create in Pushover.

**How to find it:**

| In Pushover, go to "Your Applications" and copy the API Token for the NP Companion application you created. |
| :---- |
...
All of NP Companion's code will live in one folder. Every file you create during this project goes inside this folder.

36. On your personal computer (not the work PC), open File Explorer.

37. Navigate to your Documents folder.

38. Create a new folder called: np-companion

39. Open VS Code.

40. Click File \> Open Folder and select the np-companion folder you just created.
...
These are the Python add-on libraries that give NP Companion its capabilities. Install them all now in your virtual environment so you don't have to stop and install things mid-build.
...
| ✅  CHECKPOINT — Test Before Moving On: The np-companion folder is open in VS Code config.py exists in the project root and has all sections .gitignore exists and includes config.py The venv folder exists in the project Running "pip list" in the terminal (with venv active) shows flask, playwright, pyautogui, pytesseract, and others No errors appeared during pip install |
| :---- |

# **Section 4: Project Architecture Overview**

Before building anything, understanding how all the pieces fit together will save you significant confusion later. NP Companion has three components that all talk to each other.
...
| **SQLite Database (npcompanion.db)** | Your local data store. All time logs, notes, order sets, medication reference data, and settings are saved here. Never leaves your work PC. |
...
| np-companion/ ├── app.py                  ← Main web server file ├── agent.py                ← Background desktop agent ├── config.py               ← Your machine settings (never committed to Git) ├── requirements.txt        ← List of all Python libraries ├── .gitignore │ ├── models/                 ← Database table definitions │   ├── \_\_init\_\_.py │   ├── user.py             ← Provider accounts │   ├── timelog.py          ← Visit timer records │   ├── inbox.py            ← Inbox snapshots │   ├── oncall.py           ← On-call notes │   ├── orderset.py         ← Order set definitions │   ├── medication.py       ← Medication reference │   ├── labtrack.py         ← Lab value tracking │   ├── caregap.py          ← Care gap records │   ├── tickler.py          ← Follow-up tickler │   └── message.py          ← Delayed messages │ ├── routes/                 ← Web page and API endpoints │   ├── \_\_init\_\_.py │   ├── auth.py             ← Login / logout / account management │   ├── dashboard.py        ← Today view and home │   ├── timer.py            ← Visit timer │   ├── inbox.py            ← Inbox monitor │   ├── oncall.py           ← On-call notes │   ├── orders.py           ← Order set manager │   ├── medref.py           ← Medication reference │   ├── labtrack.py         ← Lab tracker │   ├── caregap.py          ← Care gaps \+ billing │   ├── metrics.py          ← Productivity dashboard │   └── tools.py            ← Utilities (calculator, generators) │ ├── templates/              ← HTML pages │   ├── base.html           ← Shared layout (nav, header, footer) │   ├── login.html │   ├── dashboard.html │   └── \[one file per route\] │ ├── static/                 ← CSS, JavaScript, images │   ├── css/ │   │   └── main.css │   └── js/ │       └── main.js │ ├── agent/                  ← Background agent modules │   ├── mrn\_reader.py       ← OCR screen watcher │   ├── inbox\_monitor.py    ← Inbox OCR and diff tracking │   ├── scheduler.py        ← Scheduled job runner │   ├── notifier.py         ← Pushover integration │   └── pyautogui\_runner.py ← Amazing Charts automation │ ├── scrapers/               ← NetPractice browser automation │   └── netpractice.py      ← Schedule and portal scraper │ └── data/                   ← Created at runtime, excluded from Git     ├── npcompanion.db      ← SQLite database     ├── np\_session.pkl      ← NetPractice browser session     └── backups/            ← Automatic nightly backups |
| :---- |

## **4.3  External APIs**

NP Companion uses free government APIs (NIH, FDA, CMS, CDC) for clinical data enrichment. No PHI is ever sent to any external API — only clinical vocabulary terms (drug names, ICD-10 codes, LOINC codes). The full API specification, endpoint reference, caching architecture, and offline behavior are documented in **`Documents/API_Integration_Plan.md`** — the authoritative reference for all API integration.

### API Inventory

| Tier | API | Base URL | Auth | Primary Use | Key Features |
| :---- | :---- | :---- | :---- | :---- | :---- |
| **1** | **RxNorm** | `https://rxnav.nlm.nih.gov/REST/` | None | Drug normalization, brand↔generic, properties, NDC codes | F6d, F10, F10c, F10d, F10e, F25, F26, F31 |
| **1** | **RxClass** | `https://rxnav.nlm.nih.gov/REST/rxclass/` | None | Therapeutic class mapping (drug → class, class → members) | F10 (condition-first search), NEW-D |
...
| **1** | **UMLS** | `https://uts-ws.nlm.nih.gov/rest/` | Free account + key | Master terminology crosswalk (SNOMED↔ICD-10↔RxNorm↔LOINC) | F17c, F31, NEW-G |
...
All API responses are cached in local SQLite tables following the existing `Icd10Cache` pattern. Cache-first architecture: local cache is always checked before any API call. On API failure, stale cached data is returned with a staleness indicator. TTLs range from permanent (RxNorm — drugs don't change names) to 24 hours (recalls — time-sensitive). See `API_Integration_Plan.md` Section 8 for the full cache table inventory and `get_cached_or_fetch()` pattern.
...
| 💬  AI PROMPT — Copy this into Claude in VS Code: Add a notification preferences page at /settings/notifications with the following per-user settings: \- Pushover enabled (boolean) \- Inbox check interval (dropdown: 30min, 1hr, 2hr, 4hr) \- Quiet hours start (time picker, default 10 PM) \- Quiet hours end (time picker, default 7 AM)   \- Weekend alerts enabled (boolean) \- Critical value alerts (always on, cannot be disabled) \- Which notification types to receive (checkboxes: new labs, new radiology, new messages, end-of-day reminder, morning briefing)   Save all preferences to the user's preferences JSON column. The scheduler reads these preferences per user before sending any notification. |
| :---- |

### **Feature 1e: Session Timeout & Auto-Lock**

Room computers are shared. If you walk out of the room and leave NP Companion open, the screen should lock automatically to prevent anyone from seeing your patient data.

| 💬  AI PROMPT — Copy this into Claude in VS Code: The auto-lock system in main.js should: 1\. Track last user activity (any mouse move, keypress, or click resets the timer) 2\. After INACTIVITY\_TIMEOUT minutes (default 5, configurable per user), display a full-screen overlay 3\. The overlay shows only a PIN entry field (4 digits) and the NP Companion logo 4\. Correct PIN dismisses the overlay 5\. After 3 incorrect PIN attempts, the overlay requires a full login 6\. The PIN is set in Account Settings, hashed and stored in the user record 7\. Add a POST /api/verify-pin endpoint that checks the PIN without exposing the hash   The overlay should be a fixed-position div with z-index 9999 and a blurred background, not a separate page. |
| :---- |
...
| 💬  AI PROMPT — Copy this into Claude in VS Code: Add crash recovery to agent.py:   1\. On startup, check the database for any time log entries with a session\_start but no session\_end from the last 24 hours 2\. For each incomplete entry, create a popup notification asking the provider to confirm the session end time 3\. If they don't respond within 10 minutes, mark the entry with a 'duration\_estimate' flag and close it using the last heartbeat time as the approximate end 4\. Write a startup log entry to the database including the PID, startup time, and whether a crash recovery was performed 5\. Add a try/except wrapper around the main agent loop that catches any exception, logs it with full traceback to the database, attempts to restart the failed component, and sends a push notification to the admin user |
| :---- |

| ✅  CHECKPOINT — Test Before Moving On: Running "python agent.py" shows a system tray icon Right-clicking the tray icon shows the menu options The NP Companion web UI shows a green agent status dot Killing and restarting agent.py triggers crash recovery notification The /admin/agent page shows uptime and job schedule Logging in and out of NP Companion updates the active\_user.json file |
| :---- |

| PHASE 2: Data Layer — NetPractice Scraper & Inbox Monitor | \~12-16 hours |
| :---- | :---: |
...
| 💬  AI PROMPT — Copy this into Claude in VS Code: In the schedule scraper and Today View: 1\. Add a is\_new\_patient field to the Schedule model (boolean) 2\. In the scraper, detect new patient appointments (NetPractice likely uses a different visit type label or a visual indicator \- flag any visit typed as "New Patient" or "NP" or "First Visit") 3\. In the Today View template, show a gold "NEW" badge next to new patient entries 4\. Add a count of new patients to the morning briefing |
| :---- |
...
| 💬  AI PROMPT — Copy this into Claude in VS Code: Add re-auth monitoring: 1\. The scraper's is\_session\_valid() check runs before every scrape 2\. If session is invalid, immediately:    a. Set user preference needs\_reauth=True in database    b. Send Pushover notification: "NetPractice needs re-authentication. Please remote in via Splashtop and log in."    c. Show a banner in NP Companion's Today View     d. Skip the scrape attempt (don't try to run without auth) 3\. After successful re-auth, clear the flag and send a confirmation notification |
| :---- |

| ✅  CHECKPOINT — Test Before Moving On: Running the scraper manually prompts for login on first run After login, the session file exists at data/np\_session.pkl Running the scraper a second time does NOT prompt for login Tomorrow's schedule appears in the database (visible in DB Browser) The Today View shows tomorrow's schedule with visit types New patient appointments show the gold NEW badge Deleting np\_session.pkl and running again triggers the re-auth notification |
| :---- |

### **Feature 4f: Direct Patient Search (Off-Schedule)**

The NP Companion dashboard must support searching for any patient by name or MRN, not just patients on today's schedule. This is needed for:
...
1. **Primary** — `win32gui.GetWindowText()` on the foreground window, regex `ID:\s*(\d+)` to extract MRN
2. **Fallback** — OCR on the pink patient context bar in the inbox preview panel
3. **Last resort** — Tesseract region OCR only if both above methods return None
...
The inbox is always visible on the AC home screen — no keyboard shortcut or navigation required. The monitor cycles through the filter dropdown (7 options) and OCR-reads the `From | Subject | Received` table for each filter.
...
| 💬  AI PROMPT — Copy this into Claude in VS Code: Enhance the inbox monitor with tiered notification logic: 1\. Critical detection: Any item whose OCR text contains critical value keywords triggers an IMMEDIATE Pushover notification with sound 'siren', priority 1 (bypasses quiet hours) 2\. Normal new items: Batched into the regular scheduled notification 3\. The critical notification message: "🚨 CRITICAL VALUE in inbox \- Log in immediately to review" 4\. Log each critical detection with timestamp in the database for the audit trail |
| :---- |
...
| 💬  AI PROMPT — Copy this into Claude in VS Code: Add manual MRN entry to the Timer module: 1\. In the Timer page, add a "Manual Entry" button 2\. A form appears with: MRN field, session start (datetime picker), session end (datetime picker), visit type dropdown, notes 3\. Submitting creates a TimLog entry with a manual\_entry=True flag 4\. Manual entries show a pencil icon in the audit log to distinguish them from automatically captured sessions 5\. Manual entries can be edited or deleted (auto-captured entries cannot be deleted, only annotated) |
| :---- |

| ✅  CHECKPOINT — Test Before Moving On: Opening a patient chart in Amazing Charts creates a new TimLog entry in the database Switching to a different patient chart creates a new entry and closes the previous one Closing Amazing Charts closes the active time log entry The calibration mode confirms win32gui is correctly identifying the AC chart window and extracting MRN from the title bar The fallback OCR region reads correctly when title bar extraction fails Leaving the computer idle for 5 minutes pauses the timer The Timer page in NP Companion shows the current active MRN and session duration |
| :---- |

### **Feature 6d: Clinical Summary Exporter & Parser**

A new agent module (`agent/clinical_summary_parser.py`) that automates exporting and parsing the Amazing Charts Clinical Summary XML. This provides structured patient data (medications, diagnoses, allergies, vitals, labs, immunizations, demographics) without OCR — directly from the XML export.

**Critical constraint — two-phase workflow:** The Clinical Summary export is a two-phase batch process. **Phase 1 (Chart Opening):** For every patient on the schedule, search by patient ID in the blue Patient List field (under "Schedule", "Messages", "Reports", "Secure" buttons), verify the name, double-click to open the chart, select the **Visit Template** radio button, click **Select Template → Procedure Visit → Companion**, clear any popups, then press **Ctrl+S** to save/close (sends the note to the inbox). Repeat for ALL patients. **Phase 2 (XML Export):** Only after all charts are in the inbox — ensure the AC home screen is active (no chart windows open), find each patient's most recent chart in the inbox (verify by time column, name, and MRN), single-click to select, then navigate `Patient menu (Alt+P) > Export Clinical Summary`, select **"Full Patient Record"** from the encounter dropdown (never use a single encounter), verify all checkboxes and destination folder, click Export, dismiss the success dialog. Repeat for ALL patients.
...
| ⚠️  RxNorm Integration: The CDA XML embeds RxNorm CUI codes in medication strings (e.g., "Lipitor 20 mg tablet [RxNorm: 617310]"). The parser's `_parse_code_from_text()` function already extracts these codes. The `rxnorm_cui` value must be saved to the `PatientMedication.rxnorm_cui` column (not discarded). After parsing, the enrichment function `_enrich_rxnorm(rxcui)` in patient.py resolves each RXCUI into clean brand name, generic name, dose, and form via the NIH RxNorm API — cached locally in the `RxNormCache` table so each drug is only looked up once. If no RXCUI is present in the XML, the fallback uses `findRxcuiByString` (approximate string match) against the raw drug name. |
| :---- |

| ✅  CHECKPOINT — Test Before Moving On: Exporting a Clinical Summary from AC creates a file in data/clinical\_summaries/ The parser extracts medications, diagnoses, allergies, and vitals from the XML Parsed data appears in the NP Companion database The file watcher detects new XML files automatically The AuditLog shows a record for each parse operation The auto-deletion job removes files older than 183 days Medications in the database have `rxnorm_cui` populated from the XML RxNorm-enriched medications show clean brand/generic names in the patient chart |
| :---- |
...
| 💬  AI PROMPT — Copy this into Claude in VS Code: Add callback tracking to the on-call system: 1\. In oncall\_list.html, add a dedicated "Callbacks Due" section at the top showing any promised callbacks past their due time 2\. The morning briefing notification includes: "On-call callbacks overdue: X" 3\. A push notification fires 30 minutes before a promised callback time: "Callback due in 30 minutes for \[patient identifier\]" 4\. The /oncall dashboard shows a count badge for overdue callbacks |
| :---- |
...
| 💬  AI PROMPT — Copy this into Claude in VS Code: Add a handoff summary feature: 1\. GET /oncall/handoff \- Generates a summary of all notes from the current call period with status 2\. The summary contains ONLY: call times, chief complaints (no patient identifiers), and statuses 3\. A "Share Handoff" button generates a temporary 1-hour link that a colleague can open in their NP Companion 4\. The link shows the handoff summary but does not require login (time-limited, read-only) |
| :---- |
...
| 💬  AI PROMPT — Copy this into Claude in VS Code: Add shared annotations: 1\. Each medication entry can have both personal notes (visible only to the author) and shared notes (visible to all practice users) 2\. Shared notes show the author's initials and date 3\. An admin can delete inappropriate shared annotations 4\. Examples: "Blue Cross Step 1 requires this before approving drug X" or "Our pharmacy doesn't carry brand-name Z" |
| :---- |
...
| �  RxNorm Enhancement: `getRxcuiHistoryStatus` detects when a drug's RXCUI has been obsoleted or remapped by the NLM (e.g., drug withdrawn from market, name changed, reformulated). `findActiveProducts` finds current active replacements. This enables automatic flagging of outdated medication entries in the reference — entries whose RXCUI status has changed are auto-flagged for review without manual intervention. |
| :---- |

| �💬  AI PROMPT — Copy this into Claude in VS Code: Add guideline review tracking: 1\. Each entry has a last\_reviewed\_date and a guideline\_review\_flag boolean 2\. Admin can flag any entry as "recommend review" 3\. Flagged entries show a yellow banner: "This entry has not been reviewed since \[date\]. Verify against current guidelines." 4\. A /medref/review-needed page lists all flagged entries for bulk review and updating |
| :---- |

| ✅  CHECKPOINT — Test Before Moving On: The Medication Reference page loads and shows seeded conditions Searching "hypertension" returns relevant drug entries First-line filter shows only first-line options Pregnancy filter hides contraindicated entries The popup window opens at /medref/popup The AutoHotkey script opens the popup window on hotkey press Adding a personal annotation saves and displays correctly |
| :---- |

### **Feature 10e: Patient Chart View**

NP Companion's most important display feature. A dedicated patient chart view accessible from:
1. The Today View — click any patient name to open their chart
2. The dashboard search bar — search by name or MRN (Feature 4f)

**Why this feature exists:** Amazing Charts requires 8+ clicks to navigate between medications, labs, and diagnoses for a single patient. The patient chart view in NP Companion presents all of this information simultaneously in one browser window, loaded from the parsed Clinical Summary XML, with zero AC navigation required. The Prepped Note tab allows pre-charting before the visit and direct injection into AC when ready.

| 🔗  RxNorm-Enriched Medications Widget: The Medications tab and the Active Medications overview widget display data enriched by the RxNorm API. Instead of showing the raw XML string (e.g., "Lipitor 20 mg tablet [RxNorm: 617310]"), the widget shows clean columns: **Brand Name** (Lipitor), **Generic Name** (atorvastatin), **Dose + Form** (20 mg tablet), **Frequency**, **Status**. The UpToDate hyperlink uses the generic name for more reliable lookups. All enrichment data comes from the `RxNormCache` table — populated at XML parse time, so the chart loads instantly with no API delay. |
| :---- |

**Chart view layout — Left sidebar tabs (mirrors AC section structure):**
```
📋 Overview         ← landing page with customizable widgets
💊 Medications      ← active med list, last updated timestamp
🔬 Labs             ← lab results with trend indicators (links to F11)
🩺 Diagnoses        ← active problem list with ICD-10 codes
💉 Immunizations    ← immunization history from XML
📊 Vitals           ← most recent vitals + historical trend
⚠️ Allergies        ← allergy list with reactions
📝 Prepped Note     ← note writing workspace (see below)
📁 Care Gaps        ← links to F15 care gap tracker for this patient
```

**Overview tab — customizable widgets:**
...
- `Copy All to Clipboard` — formats all sections and copies for AC paste
...
| ✅  CHECKPOINT — Test Before Moving On: The patient chart view loads for a patient with parsed Clinical Summary data All sidebar tabs display correct data from the database The Prepped Note tab shows 16 section textareas in the correct order Copy All to Clipboard formats and copies all sections correctly Save Draft persists note content to the database The "Load from Amazing Charts" button triggers the export automation The search bar in the dashboard header returns matching patients MRN displays as last-4 only on all visible UI elements |
| :---- |
...
| 💬  AI PROMPT — Copy this into Claude in VS Code: Add visit type auto-tagging: 1\. When the MRN reader opens a new session, immediately check the Schedule table for an appointment today matching this patient 2\. If found, copy the visit\_type from the schedule to the time log entry 3\. The visit type is editable in the timer UI in case the auto-tag is wrong 4\. A confidence indicator shows whether the tag was auto-assigned or manually set |
| :---- |
...
| 💬  AI PROMPT — Copy this into Claude in VS Code: Add wellbeing trend monitoring: 1\. Weekly calculation of three indicators:    \- After-hours chart time: minutes of chart sessions outside 7 AM \- 6 PM    \- Inbox backlog trend: is the inbox item count growing week over week    \- Face-to-face ratio trend: is your documentation time growing relative to patient time 2\. If any indicator worsens for 3 consecutive weeks, a discrete note appears in your weekly summary: "Documentation time has increased 15% over the past 3 weeks." 3\. NOT an alert — just information presented matter-of-factly in the weekly report |
| :---- |
...
| 💬  AI PROMPT — Copy this into Claude in VS Code: When a care gap is marked as addressed: 1\. Show a documentation snippet pre-generated from the gap's template 2\. Example: "Colorectal cancer screening discussed. Patient agrees to colonoscopy referral. Order placed today." 3\. A copy button puts this text in the clipboard for pasting into Amazing Charts 4\. The gap record is updated with: completed\_date, documentation\_snippet, the provider who addressed it |
| :---- |
...
| 💬  AI PROMPT — Copy this into Claude in VS Code: Create the End-of-Day Checker as a scheduled agent job.   agent/eod\_checker.py: A function run\_eod\_check(user\_id) that: 1\. Checks for unsigned notes in Amazing Charts (via OCR scan of the notes section) 2\. Checks for unreviewed inbox items (from the last InboxSnapshot) 3\. Checks for overdue tickler items 4\. Checks for on-call notes still marked as pending documentation 5\. Checks for any messages in the delayed queue that will send tonight 6\. Compiles a summary and sends a push notification and/or desktop notification   The notification format: "End of Day Check \- \[TIME\] • Unsigned notes: X • Unreviewed inbox items: X • Overdue follow-ups: X • Pending on-call documentation: X All clear: YES / ACTION NEEDED"   A GET /tools/eod route shows the same information as a web page. Schedule this job at a user-configurable time (default 4:45 PM on workdays). |
| :---- |
...
| 💬  AI PROMPT — Copy this into Claude in VS Code: In the EOD checker, prioritize unsigned note detection: 1\. The unsigned note check navigates to Amazing Charts' note management section 2\. Uses OCR to count notes marked as unsigned or pending 3\. If any unsigned notes exist, the push notification uses priority=1 (high priority, bypasses quiet hours) 4\. The EOD web page highlights unsigned notes in red at the top of the list 5\. A reminder fires again 30 minutes before clinic closing if notes remain unsigned |
| :---- |
...
| 💬  AI PROMPT — Copy this into Claude in VS Code: Add commute mode: 1\. GET /briefing/commute \- A page optimized for text-to-speech 2\. Uses the browser's Web Speech API (SpeechSynthesis) to read the briefing aloud 3\. Auto-plays when the page loads (with a play/pause button) 4\. Reads in a logical conversational order: "Good morning. You have 14 patients today. Three are new patients. Your first appointment is at 8 AM..." 5\. An "Add to Home Screen" prompt guides mobile users to add this as a shortcut for easy pre-commute access |
| :---- |

## **FEATURE 23: AutoHotkey Macro Library**

Text expansion hotkeys for Amazing Charts. Type a short code and it expands to a full template. Works inside Amazing Charts independently of the NP Companion web server.

| 💬  AI PROMPT — Copy this into Claude in VS Code: Create the AutoHotkey macro system with NP Companion integration.   Part 1 \- macros/np\_companion.ahk (the AutoHotkey script): Create a comprehensive AutoHotkey v2 script that: 1\. Defines hotkey expansions for common clinical text (examples below) 2\. Reads macro definitions from a JSON file (macros/macros.json) so they can be edited in NP Companion without touching the .ahk file 3\. Hotkeys:    ;;ros → Full 10-system Review of Systems template    ;;penorm → Physical exam within normal limits template    ;;f/u → Standard follow-up instructions    ;;noshow → No-show note template    ;;ref → Referral order template    ;;htn → HTN counseling boilerplate    ;;dm → DM2 education documentation    ;;ama → Against Medical Advice template    Win+M → Open medication reference popup    Win+C → Open coding suggester popup    Win+O → Open order set manager 4\. A system tray icon showing macro status (active/paused) 5\. A hotkey (Win+P) to pause/resume all macros   Part 2 \- NP Companion macro management: routes/tools.py \- Add: \- GET /macros \- Macro library management page \- POST /macros/add \- Add new macro \- POST /macros/\<id\>/edit \- Edit macro \- POST /macros/\<id\>/delete \- Delete macro   The AutoHotkey script reloads macros.json automatically whenever it changes (file watcher). |
| :---- |
...
**Logic flow:**
1. Daily at 5:45 AM — query all unique RXCUI values across the patient medication database
...
4. Morning briefing: "FDA Recalls affecting patients: [N] — review list"
...
**Logic flow:**
1. On new lab result import from XML — check against LOINC reference range AND provider's custom threshold
2. If outside range: query patient's medication list, check each drug's FDA label for monitoring parameters
3. Generate contextual text: "[Lab] is [direction] — patient is on [drug] which [context]. Consider: [action]."
...
**Logic flow:**
1. When chart opens — identify top 3 active diagnoses (by encounter recency)
...
**Logic flow:**
1. For each active diagnosis → RxClass returns expected drug classes
2. Check patient med list for any medication in each expected class
3. If none found for chronic condition with >1 encounter → flag as gap
4. Display: "No [drug class] found for [diagnosis] — verify treatment plan"
5. Dismissible with note (e.g., "managed with lifestyle modification") — does not re-appear
...
**Triggers:**
- Care gap addressed → MedlinePlus content for that topic → draft message in queue
- New medication detected in pre-visit prep → MedlinePlus drug info → draft message
...
**Display sections:**
- **Interactions** — from FDA label `drug_interactions` field, cross-referencing full med list
- **Recalls** — from OpenFDA Recalls matching patient's current medications
- **Monitoring Due** — from FDA label monitoring requirements + Lab Tracker dates

Each item has a "Document Reviewed" button → audit trail entry for liability documentation.
...
**Dependencies:** ICD-10 API, UMLS API, NLM Conditions API

**Display sections:**
- **High Consideration** — conditions already in assessment
- **New Differentials** — from NLM Conditions API queried against chief complaint
- **Red Flags** — hardcoded per chief complaint category (chest pain, dyspnea, headache, etc.). These are prompts to consider, not diagnoses, and are always appropriate.

| ⚠️  Clinical framing: The differential widget is labeled "Clinical Reference Only" — it does not diagnose. Red flags are hardcoded safety prompts, not API-derived, and are never wrong because they prompt consideration. All API-derived suggestions are labeled "NLM Clinical Conditions" with source attribution. |
| :---- |
...
| PHASE 10B: Billing Intelligence Layer — Revenue Opportunity Engine | \~16-20 hours |
| :---- | :---: |
...
**Annual maintenance:** Billing rules are CMS policy — hard-coded, not API-driven. System generates admin reminder on November 1 to review and update rules when CMS publishes the new PFS Final Rule.

| ✅  CHECKPOINT — Phase 10B: `BillingOpportunity` and `BillingRuleCache` tables exist. CMS PFS data loaded for current year. Pre-visit billing cards appear in Today View with estimated revenue. CCM eligibility flags for patients with ≥2 chronic conditions. AWV sequence logic correctly identifies G0402/G0438/G0439. G2211 add-on reminder appears for established patient visits. TCM window opens on hospital discharge detection (with priority push notification). Post-visit billing review shows captured vs. missed opportunities. `/settings/billing` and `/admin/billing` pages functional. Revenue estimates labeled "approximate" with payer caveats. |
| :---- |

# **Section 6: Deploying to Your Work PC**

Once a phase is complete and working on your personal computer, follow these steps to deploy it to your work PC. You will do this incrementally — deploy after each phase, not at the very end.
...
| cd np-companion git init git add . git commit \-m "Initial project setup" |
| :---- |

53. Create a free private repository on GitHub (github.com). Name it np-companion. Do NOT make it public.
...
| git clone https://github.com/\[your-username\]/np-companion |
| :---- |

## **6.2  Work PC Configuration**

The work PC needs its own config.py with work-PC-specific values — its own screen resolution, IP addresses, and paths.

57. Copy your config.py to the work PC (do this manually via USB drive or email — do NOT push config.py to GitHub).
...
| cd np-companion python \-m venv venv .\\venv\\Scripts\\activate pip install \-r requirements.txt playwright install chromium |
| :---- |
...
| schtasks /create /xml agent\_startup.xml /tn "NP Companion Agent" |
| :---- |
...
62. Set both tasks to: Trigger \= At log on, Run whether user is logged in or not (uncheck this — it needs the screen for OCR), Run with highest privileges.

63. Test by restarting the work PC and verifying NP Companion is accessible at localhost:5000.
...
## **7.1  Amazing Charts — Required Information**

### **MRN location and format**

✅ **ANSWERED** — MRN is in the patient chart window title bar after `ID:`. Format is numeric, length varies. Use `win32gui.GetWindowText()` + regex `ID:\s*(\d+)`. No OCR required for MRN detection. The MRN is only visible when a patient chart window is open.

### **Inbox navigation**

✅ **ANSWERED** — Inbox is in the right panel of the AC home screen — it is always visible, no keyboard shortcut needed to open it. The filter dropdown (second control in the inbox header row) has 7 exact options:
- Show Everything
- Show Charts
...
✅ **ANSWERED** — `F7` opens the Most Recent Encounter tab. The `Enlarge Textbox` button (in the toolbar row above Chief Complaint, to the left of the patient photo) opens the full-section editing window. `Update & Go to Next Field` button sequences through all 16 sections without closing the window.

### **Template selection interface**

✅ **ANSWERED** — `Patient > Print > Notes & Letters` opens the Print Encounters dialog. Header dropdown has 9 options. Most recent encounter is pre-selected. `Save Section Template` and `Save as Visit Template` buttons are available in the Enlarge Textbox window for creating and applying templates.

### **Order entry interface**

Why it matters: How do you access orders for an open patient? What are the tab names in the order entry screen? How do you select an individual order — is it a checkbox, a button, a double-click?
...
✅ **ANSWERED** — Patient List panel on left side of the AC home screen. Type MRN in the `ID` column search field for fastest lookup. Search by Lastname/Firstname also available. Click patient row to open chart.

### **AC version and database location**

⏳ **OPEN** — Confirm via Help > About Amazing Charts.

### **Amazing Charts process name**

⏳ **OPEN** — Confirm via Task Manager (Ctrl+Shift+Esc → Processes tab). Note the exact name in the Name column.
...
✅ **ANSWERED** — Two confirmed methods:
1. `Patient > Print > Notes & Letters` → PDF (OCR fallback for narratives)
2. `Patient > Export Clinical Summary` → XML (preferred, structured data)

XML file auto-named `ClinicalSummary_PatientId_[MRN]_[YYYYMMDD]_[HHMMSS].xml`. Export requires the AC home screen — all patient chart windows must be closed first.

## **7.2  NetPractice / WebPractice — Required Information**
...
## **7.3  Splashtop — Required Information**
...
When something doesn't work, use this section before asking Claude for help. Paste the exact error message into Claude along with the phrase "I am building NP Companion. Here is the error:" — this gives Claude the context needed to help efficiently.
...
70. Make sure you are in the np-companion folder when running python app.py
...
72. The image may need preprocessing — increase contrast or scale before OCR

73. Tesseract may not be in your PATH — verify with: tesseract \--version in Command Prompt

74. The MRN text may be too small — increase the capture region size
...
75. Screen resolution may have changed — re-run calibration

76. Amazing Charts window may not be at the expected position — make sure it is maximized before running automation
...
80. The session cookie may have expired — this happens every 2-4 weeks
...
82. Check the Notifications log in NP Companion — was the notification sent?
...
86. Delete data/npcompanion.db and restart the app — this recreates all tables (you will lose any test data)
...
*Symptom: Can't access NP Companion from phone*
...
# **Section 9: Master Build Checklist**

Use this checklist to track your overall progress. Check off each item as it passes its checkpoint test.

> **This table must be updated after every prompt.** Status definitions:
> - **Done + Verified (User)** = You tested it yourself and confirmed it works
> - **Done + Verified (AI)** = AI ran import/route/logic checks and confirmed it works
> - **Done — Needs Testing** = Code exists but hasn't been tested yet
> - **Done — Issues Found** = Code exists but known problems remain
> - **In Progress** = Actively being worked on
> - **Not Started** = No code written yet

| Phase | Feature | Description | Build Status | AI Tested | User Verified |
|-------|---------|-------------|-------------|-----------|---------------|
| **1** | F1 | Project Skeleton & GUI Shell | Done — Needs Testing | ✅ Imports OK | ⬜ |
| **1** | F1a | Multi-User Account System | Done — Needs Testing | ✅ Imports OK | ⬜ |
| **1** | F1b | Role-Based Access | Done — Needs Testing | ✅ Imports OK | ⬜ |
| **1** | F1c | Dark Mode / Light Mode | Done — Needs Testing | ✅ Imports OK | ⬜ |
| **1** | F1d | Notification Preferences Per User | Done — Needs Testing | ✅ Imports OK | ⬜ |
| **1** | F1e | Session Timeout & Auto-Lock | Done — Needs Testing | ✅ Imports OK | ⬜ |
| **1** | F1f | Audit Trail for App Access | Done — Needs Testing | ✅ Imports OK | ⬜ |
| **1** | F2 | Local Database Schema (15 tables) | Done — Needs Testing | ✅ Imports OK | ⬜ |
| **1** | F3 | Desktop Background Agent | Done — Needs Testing | ✅ Imports OK | ⬜ |
| **1** | F3a | Agent Health Monitor | Done — Needs Testing | ✅ Imports OK | ⬜ |
| **1** | F3b | Per-Provider Agent Profiles | Done — Needs Testing | ✅ Imports OK | ⬜ |
| **1** | F3c | Crash Recovery & Resume | Done — Needs Testing | ✅ Imports OK | ⬜ |
| **2** | F4 | NetPractice Schedule Scraper | Done — Needs Testing | ✅ Imports OK | ⬜ |
| **2** | F4a | New Patient Flag | Done — Needs Testing | ✅ Imports OK | ⬜ |
| **2** | F4b | Visit Duration Estimator | Done — Needs Testing | ✅ Imports OK | ⬜ |
| **2** | F4c | Double-Booking & Gap Detector | Done — Needs Testing | ✅ Imports OK | ⬜ |
| **2** | F4e | Re-Authentication Watchdog | Done — Needs Testing | ✅ Imports OK | ⬜ |
| **2** | F4f | Direct Patient Search (Off-Schedule) | Done — Needs Testing | ✅ Routes + Imports OK | ⬜ |
| **2** | F5 | Inbox Monitor with Diff Tracking | Done — Needs Calibration | ✅ Imports OK | ⬜ Needs calibration |
| **2** | F5b | Critical Value Visual Flag | Done — Needs Calibration | ✅ Imports OK | ⬜ Needs calibration |
| **2** | F5c | Held Item Registry | Done — Needs Testing | ✅ Routes OK | ⬜ |
| **2** | F5d | Inbox Age Tracker | Done — Needs Testing | ✅ Routes OK | ⬜ |
| **2** | F5e | Inbox Digest Report | Done — Needs Testing | ✅ Imports + Routes OK | ⬜ |
| **2** | F5f | Inbox Review Timestamp Log | Done — Needs Testing | ✅ Routes OK | ⬜ |
| **2** | F6 | MRN Screen Reader | Done — Needs Calibration | ✅ Imports OK | ⬜ Needs calibration |
| **2** | F6a | Chart Open Duration Warning | Done — Needs Testing | ✅ Imports OK | ⬜ |
| **2** | F6b | Idle Detection | Done — Needs Testing | ✅ Imports OK | ⬜ |
| **2** | F6c | Manual MRN Entry Override (Timer Page) | Done — Needs Testing | ✅ Routes OK | ⬜ |
| **2** | F6d | Clinical Summary Exporter & Parser | Done — Needs Calibration | ✅ Imports OK | ⬜ Needs calibration |
| **3** | F7 | On-Call Note Keeper | Done — Needs Testing | ✅ Routes + Imports OK | ⬜ |
| **3** | F7a | Voice-to-Text Input | Done — Needs Testing | ✅ Web Speech API | ⬜ |
| **3** | F7b | Call Callback Tracker | Done — Needs Testing | ✅ Routes OK | ⬜ |
| **3** | F7d | Colleague On-Call Handoff | Done — Needs Testing | ✅ Routes OK | ⬜ |
| **3** | F7e | Note Status Tracking | Done — Needs Testing | ✅ Routes OK | ⬜ |
| **3** | F8 | Order Set Manager & Executor | Done — Needs Testing | ✅ Routes + Migration OK | ⬜ |
| **3** | F8a | Shared Order Sets | Done — Needs Testing | ✅ Routes OK | ⬜ |
| **3** | F8b | Order Set Version History | Done — Needs Testing | ✅ Routes OK | ⬜ |
| **3** | F8d | Pre-Execution Confirmation Screen | Done — Needs Testing | ✅ Routes OK | ⬜ |
| **3** | F8e | Partial Execution Recovery | Done — Needs Testing | ✅ Routes OK | ⬜ |
| **3** | F9 | Pre-Visit Note Prep | Not Started | ⬜ | ⬜ |
| **3** | F9a | Template Source Sync | Not Started | ⬜ | ⬜ |
| **3** | F9c | Prep Status Dashboard | Not Started | ⬜ | ⬜ |
| **3** | F9d | Failed Prep Alert | Not Started | ⬜ | ⬜ |
| **3** | F9e | MA Prep Handoff | Not Started | ⬜ | ⬜ |
| **3** | F10 | Medication Initiation Reference | Done — Needs Testing | ✅ Imports OK | ⬜ |
| **3** | F10a | Shared Formulary Notes | Done — Needs Testing | ✅ Imports OK | ⬜ |
| **3** | F10c | Pregnancy & Renal Quick Filter | Not Started | ⬜ | ⬜ |
| **3** | F10d | Recent Guideline Update Flag | Not Started | ⬜ | ⬜ |
| **3** | F10e | Patient Chart View | Not Started | ⬜ | ⬜ |
| **4** | F11 | Lab Value Tracker | Done — Needs Testing | ✅ Routes + Migration OK | ⬜ |
| **4** | F11a | Trend Visualization | Done — Needs Testing | ✅ Chart.js + JSON endpoint | ⬜ |
| **4** | F11b | Custom Alert Thresholds Per Patient | Done — Needs Testing | ✅ Critical/Alert thresholds | ⬜ |
| **4** | F11c | Overdue Lab Notification | Done — Needs Testing | ✅ Scheduler + Pushover | ⬜ |
| **4** | F11d | Lab Panel Grouping | Done — Needs Testing | ✅ 6 standard panels | ⬜ |
| **4** | F11e | Clinical Summary Auto-Archive | Done — Needs Testing | ✅ Scheduler cron job | ⬜ |
| **4** | F12 | Visit Timer & Face-to-Face Timer | Done — Needs Testing | ✅ Routes + Migration OK | ⬜ |
| **4** | F12a | Visit Type Auto-Tag | Done — Needs Testing | ✅ Schedule auto-match | ⬜ |
| **4** | F12b | Time Complexity Flag | Done — Needs Testing | ✅ Toggle + notes | ⬜ |
| **4** | F12c | Face-to-Face Widget (Room PCs) | Done — Needs Testing | ✅ No-auth widget | ⬜ |
| **4** | F13 | Productivity & Task Time Dashboard | Done — Needs Testing | ⬜ | ⬜ |
| **4** | F13a | Benchmark Comparison | Done — Needs Testing | ⬜ | ⬜ |
| **4** | F13b | Burnout Early Warning Indicators | Done — Needs Testing | ⬜ | ⬜ |
| **4** | F13c | Weekly Summary Email | Done — Needs Testing | ⬜ | ⬜ |
| **4** | F14 | Billing Audit Log | Done — Needs Testing | ⬜ | ⬜ |
| **4** | F14a | E&M Level Calculator | Done — Needs Testing | ⬜ | ⬜ |
| **4** | F14b | Billing Anomaly Detector | Done — Needs Testing | ⬜ | ⬜ |
| **4** | F14c | Monthly Billing Summary Report | Done — Needs Testing | ⬜ | ⬜ |
| **5** | F15 | Care Gap Tracker | Done — Needs Testing | ✅ Routes + Migration OK | ⬜ |
| **5** | F15a | Age/Sex Auto-Population | Done — Needs Testing | ✅ Schedule auto-eval | ⬜ |
| **5** | F15b | Care Gap Closure Documentation | Done — Needs Testing | ✅ Doc snippets + copy | ⬜ |
| **5** | F15c | Panel-Wide Gap Report | Done — Needs Testing | ✅ Coverage % + outreach | ⬜ |
| **5** | F16 | Billing Capture Suggestions | Not Started | ⬜ | ⬜ |
| **5** | F16a | AWV Add-On Code Checklist | Not Started | ⬜ | ⬜ |
| **5** | F16b | CCM Eligibility Flag | Not Started | ⬜ | ⬜ |
| **5** | F16c | Prolonged Service Time Calculator | Not Started | ⬜ | ⬜ |
| **5** | F17 | Coding Suggester | Not Started | ⬜ | ⬜ |
| **5** | F17b | ICD-10 Specificity Reminder | Not Started | ⬜ | ⬜ |
| **5** | F17c | Code Pairing Suggestions | Not Started | ⬜ | ⬜ |
| **6** | F18 | Delayed Message Sender | Not Started | ⬜ | ⬜ |
| **6** | F18a | Recurring Message Templates | Not Started | ⬜ | ⬜ |
| **6** | F19 | Abnormal Result Response Templates | Not Started | ⬜ | ⬜ |
| **6** | F19a | Shared Template Library | Not Started | ⬜ | ⬜ |
| **6** | F20 | End-of-Day Pending Checker | Not Started | ⬜ | ⬜ |
| **6** | F20a | Unsigned Note Counter | Not Started | ⬜ | ⬜ |
| **6** | F20c | Configurable Checklist | Not Started | ⬜ | ⬜ |
| **7** | F21 | Push Notification System | Not Started | ⬜ | ⬜ |
| **7** | F21b | Escalating Alerts | Not Started | ⬜ | ⬜ |
| **7** | F22 | Morning Briefing | Not Started | ⬜ | ⬜ |
| **7** | F22a | Commute Mode | Not Started | ⬜ | ⬜ |
| **7** | F23 | AutoHotkey Macro Library | Not Started | ⬜ | ⬜ |
| **7** | F23a | Macro Sync Across Devices | Not Started | ⬜ | ⬜ |
| **7** | F24 | Follow-Up Tickler System | Not Started | ⬜ | ⬜ |
| **7** | F24a | Priority Levels | Not Started | ⬜ | ⬜ |
| **7** | F24b | MA Assignment | Not Started | ⬜ | ⬜ |
| **7** | F25 | Controlled Substance Tracker | Not Started | ⬜ | ⬜ |
| **7** | F25a | PDMP Check Due Reminder | Not Started | ⬜ | ⬜ |
| **7** | F25b | CS Patient Registry | Not Started | ⬜ | ⬜ |
| **7** | F26 | Prior Authorization Generator | Not Started | ⬜ | ⬜ |
| **7** | F26a | Shared PA Template Library | Not Started | ⬜ | ⬜ |
| **7** | F26b | PA Status Tracker | Not Started | ⬜ | ⬜ |
| **7** | F27 | Referral Letter Generator | Not Started | ⬜ | ⬜ |
| **7** | F27a | Specialty-Specific Templates | Not Started | ⬜ | ⬜ |
| **7** | F27b | Referral Tracking Log | Not Started | ⬜ | ⬜ |
| **8** | F28 | Provider Onboarding Wizard | Not Started | ⬜ | ⬜ |
| **8** | F28a | Calibration Tool for MRN Reader | Not Started | ⬜ | ⬜ |
| **8** | F28b | Starter Pack from Colleagues | Not Started | ⬜ | ⬜ |
| **8** | F29 | Practice Admin View | Not Started | ⬜ | ⬜ |
| **8** | F30 | Offline Mode | Not Started | ⬜ | ⬜ |
| **9** | F31 | Note Reformatter — Note Capture | Not Started | ⬜ | ⬜ |
| **9** | F31 | Note Reformatter — Section Parser | Not Started | ⬜ | ⬜ |
| **9** | F31 | Note Reformatter — Content Classifier | Not Started | ⬜ | ⬜ |
| **9** | F31 | Note Reformatter — Template Engine | Not Started | ⬜ | ⬜ |
| **9** | F31 | Note Reformatter — Flagged Items Review | Not Started | ⬜ | ⬜ |
| **9** | F31 | Note Reformatter — Medication Reference | Not Started | ⬜ | ⬜ |
| **9** | F31 | Note Reformatter — Diagnosis Reference | Not Started | ⬜ | ⬜ |
| **9** | F31 | Note Reformatter — User Template Builder | Not Started | ⬜ | ⬜ |
| **10A** | NEW-A | Drug Recall Alert System | Not Started | ⬜ | ⬜ |
| **10A** | NEW-B | Abnormal Lab Interpretation Assistant | Not Started | ⬜ | ⬜ |
| **10A** | NEW-C | PubMed Guideline Lookup Panel | Not Started | ⬜ | ⬜ |
| **10A** | NEW-D | Formulary Gap Detection | Not Started | ⬜ | ⬜ |
| **10A** | NEW-E | Patient Education Auto-Draft | Not Started | ⬜ | ⬜ |
| **10A** | NEW-F | Drug Safety Panel | Not Started | ⬜ | ⬜ |
| **10A** | NEW-G | Differential Diagnosis Widget | Not Started | ⬜ | ⬜ |
| **10B** | BILLING | Billing Opportunity Engine — Rule Engine | Not Started | ⬜ | ⬜ |
| **10B** | BILLING | Billing Opportunity Engine — Pre-Visit Cards | Not Started | ⬜ | ⬜ |
| **10B** | BILLING | Billing Opportunity Engine — Post-Visit Review | Not Started | ⬜ | ⬜ |
| **10B** | BILLING | Billing Opportunity Engine — Insurer Intelligence | Not Started | ⬜ | ⬜ |
| **10B** | BILLING | Billing Opportunity Engine — Scheduling Intelligence | Not Started | ⬜ | ⬜ |
| **10B** | BILLING | Settings: `/settings/billing` + `/admin/billing` | Not Started | ⬜ | ⬜ |
| **—** | DEPLOY | Work PC setup & auto-start | Not Started | ⬜ | ⬜ |
| **—** | DEPLOY | Tailscale phone access | Not Started | ⬜ | ⬜ |
| **—** | DEPLOY | Room computer access | Not Started | ⬜ | ⬜ |
| **—** | MULTI | Second provider onboarded | Not Started | ⬜ | ⬜ |
| **—** | ADMIN | Config Editor UI | Done — Needs Testing | ✅ Routes OK | ⬜ |

# **Final Notes**

| A note on scope and time: The total estimated build time for this project is 160-200 hours of active development across Phases 1-10B. Phases 10A (Intelligence Layer) and 10B (Billing Intelligence) add approximately 36-44 hours beyond the original 130-160 hour estimate. At a pace of 1-2 hours per evening and a few hours on weekends, this is approximately a 7-8 month project built incrementally. You will be using real features in Phase 2 and 3 while later phases are still being built. The API integration begins producing value the moment Phase 0.3 (RxNorm foundation) is complete — medication names normalize, drug properties populate, and each subsequent phase adds deeper intelligence. |
| :---- |

The most important instruction in this entire guide: work in small steps and test constantly. A 50-line script that you know works is more valuable than a 500-line system that you are not sure about. Every checkpoint in this guide exists to make sure you never build more than a few hours of work on top of a broken foundation.

When you get stuck — and you will — paste the error into Claude with this exact prefix: "I am building NP Companion, a clinical workflow tool for a nurse practitioner's office. Here is the error I am seeing and the relevant code:" Claude will have the context needed to help immediately.
