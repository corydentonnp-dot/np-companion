# NP Companion — Deployment & Transfer Guide
...
5. [Opening & Running NP Companion](#5-opening--running-np-companion)
6. [Updating NP Companion](#6-updating-np-companion)
...
2. An app icon (`NP_Companion.ico`) is generated if missing
3. PyInstaller packages everything into `dist\NP_Companion\`
4. A zip file is created: `dist\NP_Companion_v1.0.1.zip`
...
1. On personal PC: Upload `NP_Companion_v1.0.1.zip` to Google Drive
...
3. The zip is on the root of the USB drive (e.g. `E:\NP_Companion_v1.0.1.zip`)
...
2. Right-click `NP_Companion_v1.0.0.zip` → **Extract All**
3. Extract to: `C:\Users\FPA\Documents\NP_Companion`
...
```
NP_Companion\
├── NP_Companion.exe      ← Double-click this to run
...
TESSERACT_PATH = r"C:\Users\FPA\Documents\NP_Companion\tesseract\tesseract.exe"
...
1. Right-click `NP_Companion.exe` → **Send to** → **Desktop (create shortcut)**
2. Rename the shortcut to "NP Companion"
3. Now you can double-click the shortcut to launch

---

## 5. Opening & Running NP Companion

### Starting the app

**Double-click `NP_Companion.exe`** (or your Desktop shortcut).

What happens:
1. Flask web server starts on port 5000
2. Background agent starts (inbox monitor, scheduler, etc.)
3. A native window opens showing the NP Companion dashboard
...
- **Open NP Companion** — show the window again
...
2. **Open NP Companion** (it should already be running)
...
2. Open NP Companion → Admin → Software Updates
...
NP Companion parses CDA Clinical Summary XML files exported from Amazing Charts. There are three ways XML files can arrive:
...
- **Antivirus blocks it:** Add `NP_Companion.exe` and the `NP_Companion` folder to your antivirus exclusions (Windows Defender: Settings → Virus & threat protection → Manage settings → Exclusions → Add a folder).
- **Missing DLLs:** Make sure you extracted the entire zip, not just the .exe. The `_internal` folder must be present.

### Port 5000 already in use

Another instance may be running. Check the system tray for the blue "NP" icon and quit it first. Or open Task Manager and end `NP_Companion.exe`.
...
The zip filename must follow the pattern `NP_Companion_v1.2.3.zip` and the version number must be higher than the current version shown on the Updates page.
...
Delete the `data\npcompanion.db` file. On the next launch, a fresh database will be created and you'll go through the first-time setup again. Your config.py settings are kept.
...
```
NP_Companion_v1.0.0.zip
└── NP_Companion\
    ├── NP_Companion.exe        ← Main executable (~15 MB)
    ├── config.py               ← User settings (edit once)
    ├── NP_Companion.ico        ← App icon
...
| Launch app | Work PC | Double-click NP_Companion.exe |
