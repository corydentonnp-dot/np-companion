# CareCompanion — API Integration Plan

**Document Version:** 1.0
**Created:** CL23
**Source Reference:** `Documents/carecompanion_api_intelligence_plan.md`
**Scope:** This document defines every external API used by CareCompanion, maps each to specific features, specifies caching and offline behavior, and describes the new Intelligence Layer (Phase 10) and Billing Intelligence Layer (Phase 10B).
...
CareCompanion uses free, public government APIs for clinical data enrichment. No PHI is ever sent to any external API. All outbound requests contain only clinical vocabulary terms (drug names, ICD-10 codes, LOINC codes) — never patient identifiers, names, DOBs, or MRNs.
...
**Use case:** Powers the "condition-first search" in the Medication Reference (F10). Provider types "hypertension" → RxClass returns antihypertensive drug classes → CareCompanion displays tiered class cards with member drugs.
...
| **Auth** | ✅ Licensed 2026-03-19 (Free UMLS account + API key) |
...
**Methods implemented:**
- `search(term)` — find UMLS concepts by term (returns CUI, name, semantic type)
- `get_icd10_for_concept(cui)` — get ICD-10 codes for a CUI (sabs=ICD10CM)
- `get_snomed_for_concept(cui)` — get SNOMED CT codes for a CUI (sabs=SNOMEDCT_US)
- `resolve_abbreviation(term)` — convenience: abbreviation → canonical term + ICD-10

---

### 2.9 SNOMED CT (via UMLS)

| Field | Value |
|-------|-------|
| **Access** | Via UMLS atoms endpoint with `sabs=SNOMEDCT_US` |
| **Auth** | ✅ Same UMLS API key (approved 2026-03-19) |
| **Response** | JSON (UMLS atoms format) |
| **Cache TTL** | 30 days (shared with UMLS cache) |
| **Cache Table** | `UmlsCache` (source_vocab = 'SNOMEDCT_US') |

**Purpose:** SNOMED CT is the international clinical terminology standard. Accessed through the UMLS API rather than a separate Snowstorm server. Provides clinical concept codes that are more granular than ICD-10, enabling richer clinical documentation and decision support.

**Use cases in CareCompanion:**
- Note Reformatter (F31) — SNOMED normalization alongside ICD-10
- Coding Suggester (F17c) — SNOMED → ICD-10 crosswalk for billing code suggestions
- Care Gap Engine — SNOMED condition matching for gap rule evaluation
- Differential Diagnosis (NEW-G) — SNOMED concept relationships for differential expansion

**No PHI:** Queries contain only UMLS CUIs or clinical terms — no patient identifiers.

---

### 2.10 VSAC — Value Set Authority Center (NLM)

| Field | Value |
|-------|-------|
| **Base URL** | `https://cts.nlm.nih.gov/fhir` |
| **Auth** | ✅ Same UMLS API key (Bearer token; approved 2026-03-19) |
| **Rate Limit** | Not published |
| **Response** | FHIR R4 JSON |
| **Cache TTL** | 90 days |
| **Cache Table** | `VsacValueSetCache` |

**Key Operations:**
- `GET /ValueSet/{oid}/$expand` — expand a value set to get all codes
- `GET /ValueSet?name:contains={keyword}` — search for value sets by name

**Purpose:** VSAC provides standardized value sets used in eCQMs (electronic Clinical Quality Measures) and C-CDA documents. Each value set defines a group of codes (ICD-10, SNOMED, RxNorm, LOINC) representing a clinical concept (e.g., "Diabetes" = all diabetes ICD-10 codes).

**Use cases in CareCompanion:**
- Care Gap Engine — replace hardcoded condition rules with official CMS eCQM value set definitions
- Billing Rules — standardized condition group definitions for CCM/AWV eligibility
- Clinical Quality Measures — future eCQM reporting capability

**No PHI:** Queries contain only value set OIDs or clinical keywords.
...
| Table | Model File | Lookup Key | TTL | Status |
|-------|-----------|------------|-----|--------|
| `rxnorm_cache` | models/patient.py | RXCUI or drug name | Permanent | ✅ Implemented |
| `rxclass_cache` | models/api_cache.py | RXCUI or class ID | 90 days | ✅ Implemented |
| `fda_label_cache` | models/api_cache.py | RXCUI | 30 days | ✅ Implemented |
| `faers_cache` | models/api_cache.py | RXCUI | 30 days | ✅ Implemented |
| `recall_cache` | models/api_cache.py | RXCUI | 24 hours | ✅ Implemented |
| `icd10_cache` | models/patient.py | ICD-10 code | 365 days | ✅ Implemented |
| `loinc_cache` | models/api_cache.py | LOINC code | 365 days | ✅ Implemented |
| `umls_cache` | models/api_cache.py | CUI or term | 90 days | ✅ Implemented |
| `healthfinder_cache` | models/api_cache.py | age+sex key | 90 days | ✅ Implemented |
| `nlm_conditions_cache` | models/api_cache.py | search term | 90 days | ✅ Implemented |
| `pubmed_cache` | models/api_cache.py | diagnosis search key | 30 days | ✅ Implemented |
| `medlineplus_cache` | models/api_cache.py | ICD-10 or RXCUI | 30 days | ✅ Implemented |
| `billing_rule_cache` | models/billing.py | HCPCS/CPT code | Annual | ✅ Implemented |
...
APIs are enhancements, not dependencies. CareCompanion must function without internet access.
...
Phase 10 added a new layer of intelligence that combines API data with patient data. These are **new features** not in the original Phases 1-9. All Phase 10A features (NEW-A through NEW-F) are now implemented.
...
| `umls_api_key` | UMLS account API key | ✅ Licensed 2026-03-19 |
...
| Task | APIs | Status |
|------|------|--------|
| Save RXCUI from parser to PatientMedication | RxNorm | ✅ Complete |
| Create `RxNormCache` model | RxNorm | ✅ Complete |
| Create `api_client.py` utility with `get_cached_or_fetch()` | All | ✅ Complete |
| Migration: add `rxnorm_cui` column to patient_medications | RxNorm | ✅ Complete |

### Phase 2 (Data Layer) — Tier 1 Integration

| Task | APIs | Status |
|------|------|--------|
| RxNorm enrichment on XML import | RxNorm | ✅ Complete |
| ICD-10 validation on XML import | ICD-10 (already working) | ✅ Complete |
| `FdaLabelCache`, `FaersCache`, `RecallCache` models | OpenFDA | ✅ Complete |
| `RxClassCache` model | RxClass | ✅ Complete |

### Phase 4 (Monitoring) — Lab Intelligence

| Task | APIs | Status |
|------|------|--------|
| LOINC reference range column in Lab Tracker | LOINC | ✅ Complete |
| `LoincCache` model and migration | LOINC | ✅ Complete |

### Phase 5 (Clinical Decision Support) — Tier 2 Integration

| Task | APIs | Status |
|------|------|--------|
| HealthFinder replaces/supplements hardcoded USPSTF rules | HealthFinder | ✅ Complete |
| CDC immunization schedule data | CDC CVX | ✅ Complete |
| ICD-10 autocomplete for Coding Suggester | ICD-10 | ✅ Complete |
| UMLS crosswalk for code pairing | UMLS | ✅ Complete |
| CMS HCPCS data load for billing capture | CMS flat files | ✅ Complete |

### Phase 7 (Notifications & Briefing) — Supplementary

| Task | APIs | Status |
|------|------|--------|
| Morning briefing weather section | Open-Meteo | ✅ Complete |
| Morning briefing recall check | OpenFDA Recalls | ✅ Complete |
| Morning briefing immunization gaps | CDC CVX | ✅ Complete |

### Phase 9 (Note Reformatter) — Normalization

| Task | APIs | Status |
|------|------|--------|
| RxNorm + ICD-10 + LOINC normalization in reformatter | RxNorm, ICD-10, LOINC | ✅ Complete |
| UMLS crosswalk for vocabulary alignment | UMLS | ✅ Complete |

### Phase 10A (Intelligence Layer) — New Features

| Task | APIs | Status |
|------|------|--------|
| Drug Recall Alert System (NEW-A) | OpenFDA Recalls, RxNorm | ✅ Complete |
| Abnormal Lab Interpretation (NEW-B) | LOINC, OpenFDA Labels, RxNorm | ✅ Complete |
| PubMed Guideline Lookup (NEW-C) | PubMed E-utilities | ✅ Complete |
| Formulary Gap Detection (NEW-D) | RxNorm, RxClass, ICD-10 | ✅ Complete |
| Patient Education Auto-Draft (NEW-E) | MedlinePlus, ICD-10, RxNorm | ✅ Complete |
| Drug Safety Panel (NEW-F) | RxNorm, OpenFDA Labels, Recalls | ✅ Complete |
| Differential Diagnosis Widget (NEW-G) | ICD-10, UMLS, NLM Conditions, SNOMED CT | ⏸️ API ready, UI pending |

> **Note:** ✅ UMLS license approved 2026-03-19 — NEW-G is now fully unblocked. UMLS API key is configured in `config.py`. SNOMED CT crosswalk and VSAC value set access are also available.

### Phase 10B (Billing Intelligence) — Revenue Optimization

| Task | APIs | Status |
|------|------|--------|
| CMS PFS API integration + flat file parser | CMS PFS | ✅ Complete |
| `BillingOpportunity` + `BillingRuleCache` models | — | ✅ Complete |
| Billing rule engine (CCM/AWV/TCM/G2211/99417/BHI/RPM) | Hard-coded rules | ✅ Complete (26 detectors) |
| Insurer detection from XML demographics | — | ✅ Complete (21.1) |
| Pre-visit opportunity calculation | All billing sources | ✅ Complete |
| Today View billing cards | — | ✅ Complete |
| Post-visit billing review | — | ✅ Complete |
| Scheduling intelligence | — | ✅ Complete |
| `/settings/billing` + `/admin/billing` pages | — | ✅ Complete |
...
*This document is the authoritative reference for all external API integration in CareCompanion. It supersedes ad-hoc API references in other documents. Feature specifications in the Development Guide reference this document for API details.*
