...
consuming roughly 166 MB (from Task Manager). Keep NP Companion's memory
...
def dismiss_resurrect_dialog() -> bool:
    ...
    For NP Companion: always click 'No' — pre-visit note prep creates
...
They are internal workflow tags only. NP Companion should treat flags
...
- `DECLINED PORTAL OR NO EMAIL` should suppress any patient portal
  messaging features for that patient in NP Companion
...
**NP Companion implications:**
- The Prepped Note tab in NP Companion includes a `Confidential Information`
  section matching the AC section list
- NP Companion must enforce that only `provider` and `admin` roles can
...
```python
# Role restriction for Confidential section in NP Companion:
...
**NP Companion:** Do not build any automation or display for this tab.
...
**NP Companion vitals display:** The vitals history table is the primary
...
# For NP Companion medication display:
...
**NP Companion — Follow-Up Tickler (F24) integration:**
The Set Reminder dialog is AC's native reminder system. NP Companion's
...
# - User creation/matching when colleagues onboard to NP Companion
...
```python
# Diagnosis entry in NP Companion (Prepped Note tab):
...
**NP Companion Care Gap Tracker (F15) integration:**
...
**NP Companion — Feature 9a (Template Source Sync) implications:**
```python
...
# OR: manually populate templates from NP Companion's own template storage
```
...
# For NP Companion Feature 9a (Template Source Sync):
# If SQL database access is confirmed:
# Templates are stored in a table queryable by provider + section + name
# Read templates directly from DB for NP Companion's own template reference
...
# For NP Companion Prepped Note tab:
...
# For NP Companion Pre-Visit Note Prep (Feature 9):
...
# NP Companion should note this limitation in the prep status dashboard
...
that motivated Feature F8 in NP Companion).
...
**NP Companion Order Set Automator (Feature F8) — Critical Details:**
...
# NP Companion's F8 replaces this functionality entirely
...
# Keep NP Companion's browser footprint minimal during order automation
