...
Example observed: `3/17/2026: (forwarded)`
Also contains: `Full Patient Record` option
**NP Companion always selects "Full Patient Record"** — never a single encounter.
This ensures the exported XML contains all patient history, medications, diagnoses, etc.
...
## Enlarge Textbox Window (Note Entry)

### Observed from: enlarge_textbox_screenshot.png (3/16/2026)

**How to access:**
Click the `Enlarge Textbox` button in the Most Recent Encounter tab (F7).
The button is located in the toolbar row above the Chief Complaint field,
to the left of the patient photo. It opens a separate floating window.

### Window Title Format
```
Chief Complaint  [LASTNAME], [FIRSTNAME]  (DOB: MM/DD/YYYY; ID: [MRN])
```
Example observed:
```
Chief Complaint  TEST, TEST  (DOB: 10/01/1980; ID: 62815)
```

The title always starts with the currently selected section name. The MRN
is available in this title bar — use the same `ID:\s*(\d+)` regex.

### Window Menu Bar
```
File  |  Edit  |  Visit Template
```

### Two-Column Layout

**Left column — `Section` (list of clickable section names):**
```
Chief Complaint                  ← selected by default, highlighted yellow
History of Present Illness
Review of Systems
Past Medical History
Social History
Family History
Allergies (Open Allergy Window)
Medications (Open Med Window)
Physical Exam
...
Health Concerns
```
Clicking a section name in the left column loads that section's content
into the text area on the right. The selected section is highlighted.

**Right column — `Text`:**
Large plain-text editing area. Displays the content of whichever section
is currently selected in the left column. Editable free text.
Example observed: Chief Complaint shows `"cough" [Updated by Gretchen Lockard, MD on 1/5/2026 2:19:46 PM]`

### Bottom Action Buttons (row across the bottom of the window)
```
Close  |  Save Section Template  |  Update & Go to Prior Field  |  Update & Close  |  Update & Go to Next Field  |  Save as Visit Template
```

**Button purposes:**
- `Close` — closes the window without saving changes
- `Save Section Template` — saves current section text as a reusable template
- `Update & Go to Prior Field` — saves changes to current section, moves to previous section in list
- `Update & Close` — saves changes to current section and closes the window
- `Update & Go to Next Field` — saves changes to current section, moves to next section in list
- `Save as Visit Template` — saves ALL sections as a complete visit template

### Relevance to NP Companion

**Canonical Note Section List:**
The section list in this window is the **authoritative list of note sections**
in Amazing Charts. Use this exact list for:
- The Prepped Note tab in Patient Chart View (F10e)
- Note Reformatter (F31) section headers and ordering
- `note_parser.py` section detection patterns
- Pre-Visit Note Prep (F9) automation sequencing

**Automation via `Update & Go to Next Field`:**
This button allows sequential automation through all 16 sections without
closing the window. The pre-visit note prep automation (F9) and the
"Send to Amazing Charts" function (F10e Prepped Note tab) use this button
to paste content into each section sequentially:
1. Open Enlarge Textbox window
2. Select Chief Complaint section (it is selected by default)
3. Clear the text area, paste content for this section
4. Click `Update & Go to Next Field`
5. Repeat for each section with content
6. Click `Update & Close` on the last section

**Special sections — Allergies and Medications:**
`Allergies (Open Allergy Window)` and `Medications (Open Med Window)` open
separate dedicated windows rather than editing inline text. Automation must
handle these as separate window targets with different UI layouts. These
sections should be SKIPPED in the sequential paste flow and handled separately.

**Template mechanisms:**
`Save Section Template` and `Save as Visit Template` are the template
persistence mechanisms available in AC. Relevant to Feature 9a (Template
Source Sync). The `discover_ac_templates()` function should check both
section templates and visit templates.

### Canonical Note Section List for Python

```python
# Use this constant everywhere in NP Companion for note section ordering.
# This is the exact order from the Enlarge Textbox window in Amazing Charts.
AC_NOTE_SECTIONS = [
    "Chief Complaint",
    "History of Present Illness",
    "Review of Systems",
...
# Sections that open separate windows instead of editing inline
