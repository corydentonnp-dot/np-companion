# CareCompanion — Verification Checklist

**Last updated:** March 17, 2026 (CL22 — Clinical Summary Import & Patient Chart Widgets)

This is a **step-by-step guide** for someone who is not a programmer to verify that everything in CareCompanion is working. Follow every step in order. If something fails, stop and note which step failed.
...
You need:
- The CareCompanion folder at `C:\Users\coryd\Documents\CareCompanion`
...
2. Navigate to `C:\Users\coryd\Documents\CareCompanion`
...
1. In File Explorer, navigate to `C:\Users\coryd\Documents\CareCompanion`
...
## STEP 20 — Verify Billing Engine (Phase 13 Expansion)

**Added:** 2026-03-19

The billing engine now has 16 rule categories with VSAC-aware dynamic code lists. Provider toggles control which categories are active.

| # | Check | How to Verify |
|---|-------|---------------|
| 1 | All 16 categories exist | `venv\Scripts\python.exe -c "from app.services.billing_rules import BillingRuleEngine; e = BillingRuleEngine.__init__.__doc__; print('OK')"` → `OK` |
| 2 | Category toggles | Go to `/settings/billing` → toggle grid shows all 16 categories with on/off switches. Toggle one off, save, verify it persists on refresh |
| 3 | CCM detection | Import test patient with ≥2 chronic conditions → billing card shows CCM: 99490 |
| 4 | AWV + add-on stack | Medicare patient age 65+ with no AWV → suggests G0438/G0439 + G2211 + G0444 + G0442/G0443 + 99497 |
| 5 | Tobacco cessation | Patient with F17.* diagnosis → suggests 99406/99407 |
| 6 | Cognitive assessment | Patient age 65+ with dementia/memory diagnosis → suggests 99483 |
| 7 | Care gap → billing bridge | Patient with open care gaps → screening billing codes appear (e.g., G0105 for colonoscopy) |
| 8 | Pre-visit job | Verify the nightly 8 PM scheduled job creates `BillingOpportunity` records for tomorrow's patients |
| 9 | VSAC fallback | Disconnect internet → billing rules still work using hardcoded prefix lists |

---

## STEP 21 — Verify Intelligence Endpoints (Phase 13D)

**Added:** 2026-03-19

Five intelligence widgets are available on the patient chart. Each loads on-demand via AJAX.

| # | Check | How to Verify |
|---|-------|---------------|
| 1 | Lab Interpretation | Go to `/patient/<mrn>` → expand "Lab Interpretation" panel → should return JSON with interpretations for abnormal labs, medication cross-references |
| 2 | Drug Safety | Expand "Drug Safety" panel → should show interaction warnings, recall status, adverse event data |
| 3 | Guidelines | Expand "Guidelines" panel → should show recent PubMed guideline articles for top diagnoses |
| 4 | Formulary Gaps | Expand "Formulary Gaps" panel → should flag chronic conditions missing expected drug classes |
| 5 | Patient Education | Expand "Patient Education" panel → should show MedlinePlus content for active conditions |
| 6 | Education → Message | Click "Send to Patient" on an education result → should create a `DelayedMessage` draft and redirect to `/messages` |
| 7 | RxCUI enrichment | Patient with `rxnorm_cui` values on medications → intelligence endpoints match on therapeutic class names (from `RxClassCache`), not just drug names |

---

## STEP 22 — Verify Cache Staleness Badges (Phase 14.1)

**Added:** 2026-03-19

Cache staleness indicators show how fresh API-sourced data is.

| # | Check | How to Verify |
|---|-------|---------------|
| 1 | Med Ref badges | Go to `/medref` → search for a drug → result cards show a small badge: "Fresh" (green, <24h), "Aging" (yellow, 1-7d), or "Stale" (red, >7d) |
| 2 | Patient Detail badge | Go to `/patient/<mrn>` → the patient header area shows when the clinical summary was last parsed. If >24h ago, badge says "Stale" |
| 3 | Badge macro | The `_cache_badge.html` Jinja2 macro is reusable. Inspect page source to verify it renders `<span class="cache-badge ...">` elements |
...
4. Create an **Application** in Pushover, name it "CareCompanion"
5. Copy the **API Token** it gives you
6. Open `config.py` in the CareCompanion folder and fill in:
   ```python
...
Tailscale lets you access CareCompanion from your phone when you're not at the office.

1. Install Tailscale on your work PC: https://tailscale.com/download
2. Install Tailscale on your phone
3. Sign in with the same account on both
4. Once connected, you can access CareCompanion from your phone at:
...
4. The CareCompanion agent will use that browser session to scrape schedules
...
All commands should be run from `C:\Users\coryd\Documents\CareCompanion` in PowerShell with `(venv)` active.
