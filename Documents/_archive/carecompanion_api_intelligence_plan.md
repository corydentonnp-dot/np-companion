# CareCompanion — API Integration & Intelligence Layer
# Planning Document for VS Code Copilot (Planner Mode)
#
# This document describes every external API integrated into CareCompanion,
...
CareCompanion began as a workflow automation tool for a single family practice
...
- `GET /content/current/CUI/[cui]/atoms?sabs=ICD10CM,SNOMEDCT_US,RXNORM`
  ...
  CareCompanion's billing-focused display.
...
**Key query logic for CareCompanion:**
...
When a patient's chart is opened in CareCompanion, the system identifies
...
Understanding how data flows between these APIs and through CareCompanion
...
Every provider using CareCompanion has individual control over how APIs
...
The APIs are enhanced tools, not dependencies. CareCompanion must function
...
When a provider opens a patient chart in CareCompanion, this is the complete
...
All features described here extend the existing CareCompanion development
...
(Intelligence Layer). All intelligence features are now implemented.*
...
CareCompanion pre-calculates the full billing opportunity stack:
...
4. Load prior billing history (from CareCompanion billing audit log)
...
The system's legal framing: CareCompanion is a documentation and
...
*Billing Intelligence Layer added 3/17/2026. Implemented as Phase 19 —
Billing Capture Engine. See addendum below for full implementation status.
All dependencies (Phase 4, Phase 5, Phase 2) were completed prior to build.
Phase 10B billing features are fully operational with 26 detectors and 72 seed rules.*

---

## ADDENDUM: Billing Capture Engine — Implementation Status (Phase 19)

The billing intelligence layer described in the addendum above has been
fully implemented as a modular, auto-discovering billing capture engine in
`billing_engine/`. Phase 19 (completed) replaced the earlier monolithic
`app/services/billing_rules.py` with a scalable architecture.

### Architecture

```
billing_engine/
├── __init__.py          # Package init, re-exports BillingCaptureEngine
├── engine.py            # BillingCaptureEngine — orchestrator, auto-discovers detectors
├── base.py              # BaseDetector ABC — contract for all detector modules
├── shared.py            # SharedPatientContext dataclass — single patient data bundle
├── payer_routing.py     # classify_payer(), apply_payer_adjustments()
├── rules.py             # BILLING_RULES dict — 72 seed rule definitions
├── utils.py             # Utility helpers shared across detectors
└── detectors/           # 26 auto-discovered detector modules
    ├── __init__.py
    ├── acp.py           # Advance Care Planning (99497, 99498)
    ├── alcohol.py       # Alcohol misuse screening (G0442, G0443)
    ├── awv.py           # Annual Wellness Visit (G0402, G0438, G0439) + add-ons
    ├── bhi.py           # Behavioral Health Integration (99484)
    ├── care_gaps.py     # Dynamic care gap → billing bridge
    ├── ccm.py           # Chronic Care Management + Principal Care Management
    ├── chronic_monitoring.py  # Condition-specific monitoring codes
    ├── cocm.py          # Collaborative Care Model (99492-99494)
    ├── cognitive.py     # Cognitive Assessment (99483)
    ├── counseling.py    # Preventive counseling codes
    ├── em_addons.py     # E&M add-on detection
    ├── g2211.py         # G2211 complexity add-on
    ├── misc.py          # After-hours, phone, interprofessional consults
    ├── obesity.py       # Obesity/nutrition counseling (G0477, G0473)
    ├── pediatric.py     # 8 pediatric sub-detectors (well-child, lead, fluoride, etc.)
    ├── preventive.py    # Preventive visit detection (99381-99397)
    ├── procedures.py    # In-office procedures (spirometry, EKG, etc.)
    ├── prolonged.py     # Prolonged service (99417) time-based add-on
    ├── rpm.py           # Remote Patient Monitoring (99453-99458)
    ├── screening.py     # PHQ-9, fall risk, cognitive screening
    ├── sdoh.py          # Social determinants assessment (G0136)
    ├── sti.py           # STI screening panel codes
    ├── tcm.py           # Transitional Care Management (99495, 99496)
    ├── telehealth.py    # Telehealth modifiers and place-of-service
    ├── tobacco.py       # Tobacco cessation counseling (99406, 99407)
    └── vaccine_admin.py # 7 vaccine administration sub-detectors
```

### Engine Orchestration Flow

1. `BillingCaptureEngine.__init__()` auto-discovers all `BaseDetector`
   subclasses in `detectors/` — no registration required.
2. `engine.run(patient_data)` builds a `SharedPatientContext` from the
   patient dict (diagnoses, medications, insurer, age, sex, visit type, etc.).
3. Each detector's `detect(ctx)` method is called; returns a list of
   `BillingOpportunity`-compatible dicts.
4. `payer_routing.classify_payer()` determines insurer type
   (medicare_b, medicare_advantage, medicaid, commercial).
5. `payer_routing.apply_payer_adjustments()` filters/annotates
   opportunities based on payer eligibility rules.
6. Engine deduplicates by `opportunity_code`, sorts by priority, and
   returns the final opportunity list.

### Seed Data Summary

`billing_engine/rules.py` defines **72 billing rules** across **26 categories**
covering **122 unique CPT/HCPCS codes**. Rules are seeded into the
`billing_rule` DB table via `migrate_seed_billing_rules.py`.

| Category | Rules | CPT Codes | Detector |
|----------|-------|-----------|----------|
| acp_standalone | 1 | 99497, 99498 | acp.py |
| alcohol_screening | 1 | G0442, G0443, G2174 | alcohol.py |
| awv | 2 | G0402, G0438, G0439, G2211 | awv.py |
| bhi | 1 | 99484 | bhi.py |
| care_gap_screenings | 1 | (dynamic) | care_gaps.py |
| ccm | 2 | 99490, 99439, 99491, 99424, 99425 | ccm.py |
| chronic_monitoring | 9 | 9 codes | chronic_monitoring.py |
| cocm | 3 | 99492, 99493, 99494 | cocm.py |
| cognitive_assessment | 1 | 99483 | cognitive.py |
| counseling | 6 | 5 codes | counseling.py |
| em_addons | 1 | (modifier-based) | em_addons.py |
| g2211 | 1 | G2211 | g2211.py |
| misc | 7 | 11 codes | misc.py |
| obesity_nutrition | 1 | G0477, G0473 + 2 | obesity.py |
| pediatric | 8 | 20 codes | pediatric.py |
| preventive_visit | 1 | 99381-99397 (6) | preventive.py |
| procedures | 6 | 10 codes | procedures.py |
| prolonged_service | 1 | 99417 | prolonged.py |
| rpm | 1 | 99453-99458 (4) | rpm.py |
| screening | 3 | 4 codes | screening.py |
| sdoh | 2 | G0136, SDOH assessment | sdoh.py |
| sti_screening | 1 | 4 codes | sti.py |
| tcm | 1 | 99495, 99496 | tcm.py |
| telehealth | 3 | 7 codes | telehealth.py |
| tobacco_cessation | 1 | 99406, 99407 | tobacco.py |
| vaccine_admin | 7 | 17 codes | vaccine_admin.py |

### Database Models (models/billing.py)

- **BillingOpportunity**: Per-patient per-visit opportunity record with
  `category`, `opportunity_code`, `modifier`, `priority` (1-5),
  `documentation_checklist` (JSON), `actioned_at`, `actioned_by`.
- **BillingRule**: Reference table (72 rows) with `opportunity_code`,
  `category`, `cpt_codes` (JSON), `payer_types` (JSON),
  `estimated_revenue`, `documentation_checklist` (JSON), `is_active` toggle.

### UI Integration (Phase 19E)

- **Alert bar** (`_billing_alert_bar.html`) — high-priority billing
  badge in patient header.
- **Post-visit modal** (`_billing_post_visit.html`) — shown after timer
  stop (F12), lists opportunities with accept/dismiss actions.
- **Opportunity report** (`billing_opportunity_report.html`) — Chart.js
  dashboard at `/billing/opportunity-report` showing category breakdown,
  estimated revenue, and trend data.
- **Dashboard widget** — billing section in Today View with
  category/priority badges, checklist preview, and batch-accept button.

### Test Coverage (Phase 19F)

95 total test checks in `test.py`:
- **18 detector unit tests**: AWV±, G2211, CCM tiered/complex, PCM, BHI±,
  cognitive±, obesity, telehealth±, payer routing, dedup, priority sort,
  category toggle, graceful degradation.
- **5 seed data verification tests**: Row count ≥70, documentation checklists,
  required fields, is_active toggle, unique opportunity codes.
- **4 integration test patients**: Medicare complex (68yo → 23 opps),
  Medicaid (same patient → 20 opps, no AWV/G2211), Pediatric (12mo → 6 opps),
  Commercial (35yo → 12 opps).

---

## ADDENDUM: Implementation Status (2026-03-19)

### API Client Implementation Status

All API clients live in `app/services/api/` and follow the `BaseAPIClient` pattern with retry, rate limiting, and offline fallback.

| # | API | Client File | Status | Structured Cache |
|---|-----|-------------|--------|------------------|
| 1.1 | RxNorm | `rxnorm.py` | ✅ Fully Implemented | `RxNormCache` (models/patient.py) |
| 1.2 | RxClass | `rxclass.py` | ✅ Fully Implemented | `RxClassCache` (models/api_cache.py) |
| 1.3 | OpenFDA Drug Labels | `openfda_labels.py` | ✅ Fully Implemented | `FdaLabelCache` (models/api_cache.py) |
| 1.4 | OpenFDA Adverse Events | `openfda_adverse_events.py` | ✅ Fully Implemented | `FaersCache` (models/api_cache.py) |
| 1.5 | OpenFDA Drug Recalls | `openfda_recalls.py` | ✅ Fully Implemented | `RecallCache` (models/api_cache.py) |
| 1.6 | ICD-10 (NLM Clinical Tables) | `icd10.py` | ✅ Fully Implemented | `Icd10Cache` (models/patient.py) |
| 1.7 | LOINC | `loinc.py` | ✅ Fully Implemented | `LoincCache` (models/api_cache.py) |
| 1.8 | UMLS | `umls.py` | ✅ Fully Implemented (UMLS licensed 2026-03-19) | `UmlsCache` (models/api_cache.py) |
| 2.1 | AHRQ HealthFinder | `healthfinder.py` | ✅ Fully Implemented | `HealthFinderCache` (models/api_cache.py) |
| 2.2 | CDC Immunizations | `cdc_immunizations.py` | ✅ Fully Implemented | `CdcImmunizationCache` (models/api_cache.py) |
| 2.3 | CMS PFS | `cms_pfs.py` | ✅ Fully Implemented | `BillingRuleCache` (models/billing.py) |
| 2.4 | NLM Conditions | `nlm_conditions.py` | ✅ Fully Implemented | `NlmConditionsCache` (models/api_cache.py) |
| 3.1 | PubMed (NCBI E-utilities) | `pubmed.py` | ✅ Fully Implemented | `PubmedCache` (models/api_cache.py) |
| 3.2 | MedlinePlus | `medlineplus.py` | ✅ Fully Implemented | `MedlinePlusCache` (models/api_cache.py) |
| 4.1 | Open-Meteo | `open_meteo.py` | ✅ Fully Implemented | Runtime cache only |
| — | VSAC (UMLS-licensed) | `vsac.py` | ✅ Fully Implemented | `VsacValueSetCache` (models/api_cache.py) |

**Total: 16 API clients implemented, 15 structured cache models.**

### Intelligence Endpoints (routes/intelligence.py)

| Endpoint | Route | Status | Notes |
|----------|-------|--------|-------|
| Lab Interpretation | `GET /api/patient/<mrn>/lab-interpretation` | ✅ Implemented | Cross-references abnormal labs with patient medications. Now enriched with RxClassCache therapeutic class matching (14.5). |
| Drug Safety | `GET /api/patient/<mrn>/drug-safety` | ✅ Implemented | Aggregates interaction warnings, recall status, adverse event data |
| Guidelines | `GET /api/patient/<mrn>/guidelines` | ✅ Implemented | PubMed guideline articles for top active diagnoses |
| Formulary Gaps | `GET /api/patient/<mrn>/formulary-gaps` | ✅ Implemented | Detects chronic conditions lacking expected drug classes. Now enriched with RxClassCache therapeutic class matching (14.5). |
| Patient Education | `GET /api/patient/<mrn>/education` | ✅ Implemented | MedlinePlus content for active conditions/medications |
| Education → Message | `POST /api/patient/<mrn>/education/send` | ✅ Implemented | Auto-creates DelayedMessage draft from education content |

### RxCUI → RxClassCache Enrichment (Phase 14.5)

Both `lab_interpretation()` and `formulary_gaps()` now query `RxClassCache` for therapeutic class names when medications have `rxnorm_cui` values. Class names (e.g., "ACE Inhibitor", "Beta Blocker") are added to the medication match set, enabling broader drug-lab and drug-condition matching beyond just drug names.

### Billing Capture Engine (billing_engine/)

Modular auto-discovering engine with 26 detector modules, 72 seed rules, and 122 unique CPT/HCPCS codes. Engine auto-discovers `BaseDetector` subclasses at startup — no registration needed. Payer routing classifies insurers and adjusts opportunity eligibility. All categories provider-toggleable via `/settings/billing`. Pre-visit billing job runs nightly at 8 PM. Full architecture documented in the Phase 19 addendum above.
