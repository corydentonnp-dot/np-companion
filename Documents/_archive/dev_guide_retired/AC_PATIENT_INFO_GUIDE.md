...
The **Clinical Summary XML export** provides all structured patient data in a single machine-readable file. CareCompanion parses this file once and displays it in a unified, fast, browser-based **Patient Chart View** (Feature 10e) with zero AC navigation required.
...
# **Section 2: Data Collection — How Patient Data Enters CareCompanion**
...
The provider clicks "Load from Amazing Charts" or "Refresh from AC" on any patient chart view in CareCompanion. For a single-patient on-demand export:
...
CareCompanion uses Python's `watchdog` library to monitor the `data/clinical_summaries/` folder. When AC writes a new XML file:
...
The Patient Chart View is CareCompanion's primary patient data display. It presents all parsed Clinical Summary data in a single browser page, accessible from:
...
4. For each section in `AC_NOTE_SECTIONS` order:
   ...
   b. Clear the text area and paste the section content from CareCompanion
